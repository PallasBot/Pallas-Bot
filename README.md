...
3. 高频写入场景并发原子化（`upsert_answer` / 黑名单 / 图缓存 / 配置均走原子 `ON CONFLICT`），不再丢计数
...
6. 新增好友欢迎与群事件处理：支持自定义好友添加欢迎消息
7. 新增申请管理模块：支持自动同意好友/入群开关，以及完整的手动审批命令（查看/同意/拒绝）
8. 轮盘游戏重构：合并救援功能（`牛牛救一下`）和补枪功能（`牛牛补一枪`）
9. 新增醒酒功能：`牛牛醒一醒` / `牛牛别喝了` 可立即清除醉酒状态

### Bug 修复

10. 修复 bot 关闭时异常的 RuntimeError 报错
11. 修复 AI 服务端 `del_session` 接口路由不匹配导致的 404 错误
...
- `牛牛帮助`：在获得牛牛功能的信息的同时控制功能的开关，请先发送 `牛牛帮助`了解牛牛如何使用！
- `牛牛喝酒`：增加牛牛说话概率，有概率喝醉（）
  - `牛牛醒一醒` / `牛牛别喝了`：让牛牛立即醒酒
- `牛牛轮盘` & `牛牛开枪`：需要给牛牛管理员才能使用，试试你就知道是啥功能了.jpg
  - `牛牛救一下` / `牛牛救一下@用户`：解除被禁言的用户，有概率炸膛，喝酒后更危险
  - `牛牛补一枪` / `牛牛补一枪@用户`：延长被禁言用户的禁言时间，同样有概率炸膛
- `设置好友欢迎` / `清除好友欢迎`：牛牛管理员在私聊中设置自定义好友添加欢迎消息，支持文本、图片或图文混合
- 好友/入群管理（牛牛管理员私聊可用）：详情发送`牛牛帮助`
