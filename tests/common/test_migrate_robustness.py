...
async def test_dedupe_answers_aggregates_correctly():
    ...
    result.sort(key=itemgetter("keywords"))
    assert len(result) == 2
...
async def test_migrate_message_resumable(pg_env):
    ...
    await migrate._migrate_message(db, pg_env["sf"], MessageRow, pg_env["pg_insert"], batch_size=100, dry_run=False)
...
async def test_migrate_message_dirty_rows_counted(pg_env):
    """错类型 group_id 等经兜底后仍可入库；单条脏数据不拖垮 batch。"""
    from bson import ObjectId
    from sqlalchemy import func, select
...
async def test_migrate_bot_config_handles_auto_accept_legacy(pg_env):
    ...
    await migrate._migrate_bot_config(
        db, pg_env["sf"], BotConfigRow, pg_env["pg_insert"], batch_size=100, dry_run=False
    )
