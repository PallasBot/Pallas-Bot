...
from src.features.plugin_capabilities.schema import build_plugin_capabilities_ui
from src.features.plugin_storage.declare import plugin_storage_row
from src.features.plugin_storage.deploy_store import DeployPluginStorage, deploy_storage_path
...
def test_deploy_storage_roundtrip(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "src.features.plugin_storage.deploy_store.plugin_data_dir",
...
def test_build_plugin_capabilities_ui_groups(monkeypatch) -> None:
    ...
    monkeypatch.setattr("src.features.cmd_perm.schema.get_loaded_plugins", lambda: [FakeDraw()])
    limit_row = type("Row", (), {"id": "draw.draw", "cd_sec": 3})()
    monkeypatch.setattr(
        "src.features.command_limits.schema._all_command_limit_rows",
