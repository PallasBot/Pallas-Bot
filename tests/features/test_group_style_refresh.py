...
def test_group_style_dirty_mark_and_pop_batch() -> None:
    from src.features.persona.group_style_refresh import (
...
async def test_learner_marks_group_style_dirty_on_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.plugins.repeater.learner.group_messages_before", fake_group_messages_before)
    monkeypatch.setattr("src.plugins.repeater.learner.MessageStore.message_insert", fake_message_insert)
    monkeypatch.setattr(
        "src.features.persona.group_style_refresh.mark_group_style_dirty", lambda gid: marked.append(gid)
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "src.plugins.duel.duel_session.should_skip_repeater_learn",
...
async def test_refresh_group_style_profile_writes_profile_and_invalidates_cache(
    ...
    from src.features.persona.group_style_refresh import refresh_group_style_profile
    ...
    monkeypatch.setattr(
        "src.features.persona.group_style_refresh.make_group_config_repository", lambda: DummyGroupRepo()
    )
    monkeypatch.setattr("src.features.persona.group_style_refresh.make_message_repository", lambda: DummyMessageRepo())
    monkeypatch.setattr(
        "src.features.persona.group_style_refresh.make_local_context_repository", lambda: DummyContextRepo()
    )
    monkeypatch.setattr(
        "src.features.persona.group_style_refresh.invalidate_persona_cache",
...
async def test_refresh_dirty_group_style_batch_isolates_failures(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.features.persona.group_style_refresh.refresh_group_style_profile", fake_refresh)
    clear_group_style_dirty_state()
    mark_group_style_dirty(1)
