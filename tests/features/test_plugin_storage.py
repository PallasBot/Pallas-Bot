...
from src.features.plugin_storage.metadata import parse_plugin_storage_decl
...
async def test_persisted_group_storage(monkeypatch) -> None:
    ...
    monkeypatch.setattr("nonebot.get_loaded_plugins", lambda: [FakePlugin()])
    monkeypatch.setattr("src.foundation.config.GroupConfig._find", fake_find)
    monkeypatch.setattr("src.foundation.config.GroupConfig._update", fake_update)
    clear_plugin_storage_registry_cache()
...
async def test_undeclared_key_rejected(monkeypatch) -> None:
    monkeypatch.setattr("nonebot.get_loaded_plugins", lambda: [])
    clear_plugin_storage_registry_cache()
