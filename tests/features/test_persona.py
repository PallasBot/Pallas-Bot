...
from src.features.persona.auto import derive_persona_from_bot_id
...
async def test_resolve_persona_merges_group_style_profile(monkeypatch: pytest.MonkeyPatch) -> None:
    from src.features.persona.loader import invalidate_persona_cache, resolve_persona
    from src.features.persona.model import ResolvedPersona
...
async def test_speaker_uses_group_aware_persona(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.plugins.repeater.speaker.resolve_persona", fake_resolve_persona)
    monkeypatch.setattr("src.plugins.repeater.speaker.blocks_proactive_speak", lambda _gid: False)
    monkeypatch.setattr("src.platform.shard.registry.config.is_sharding_active", lambda: False)
    monkeypatch.setattr("src.platform.multi_bot.platform_utils.pick_connected_bot_id", lambda ids, log_tag=None: 1001)  # noqa: ARG005

    async def fake_find_ban_keywords(**_kwargs):
        return set()

    monkeypatch.setattr("src.plugins.repeater.ban_manager.BanManager.find_ban_keywords", fake_find_ban_keywords)

    async def fake_taken_name(self):
        return None

    monkeypatch.setattr("src.foundation.config.BotConfig.taken_name", fake_taken_name)
    monkeypatch.setattr("src.plugins.repeater.speaker.random.choice", itemgetter(0))
    monkeypatch.setattr("src.plugins.repeater.speaker.random.random", lambda: 1.0)
    monkeypatch.setattr("src.plugins.repeater.speaker.time.time", lambda: 10_000.0)
    monkeypatch.setattr(
        "src.plugins.repeater.speaker.Speaker._pick_speak_message",
    ...
    monkeypatch.setattr("src.plugins.repeater.speaker.MessageStore._message_dict", message_dict)
...
def test_speak_keyword_group_weight_prefers_hot_topic_when_chaotic() -> None:
    ...
    from src.features.persona.model import ResolvedPersona
    from src.features.persona.scorer import speak_keyword_group_weight
