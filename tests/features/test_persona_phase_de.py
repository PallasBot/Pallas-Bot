...
def test_build_group_style_profile_skips_contaminated_samples(monkeypatch) -> None:
    ...
    assert profile["aggregate"]["answer_count"] == 8
    assert profile["aggregate"]["contamination_skipped_messages"] == 1
    assert profile["aggregate"]["contamination_skipped_answers"] == 1
