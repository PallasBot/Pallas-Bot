...
async def test_friend_requests_overview_parallel_doubt(monkeypatch) -> None:
    monkeypatch.setattr("packages.pb_webui.social_api._read_pending_friend_requests_disk", dict)
    monkeypatch.setattr(
        mod,
        "get_bots",
        dict,
    ...
    monkeypatch.setattr("packages.pb_webui.social_api._doubt_friends_for_self_id_safe", fake_doubt)

    async def noop_enrich(*_a, **_k) -> None:
        return None

    monkeypatch.setattr("packages.pb_webui.social_api._enrich_friend_request_rows_nicknames_for_self_id", noop_enrich)
...
async def test_friend_requests_overview_self_id_filter(monkeypatch) -> None:
    monkeypatch.setattr(
        "packages.pb_webui.social_api._read_pending_friend_requests_disk",
        lambda: {"10001": {"9": "f1"}, "10002": {"8": "f2"}},
    )
    monkeypatch.setattr("packages.pb_webui.social_api.get_bots", dict)
    monkeypatch.setattr("packages.pb_webui.social_api.shard_hub_console", lambda: False)
