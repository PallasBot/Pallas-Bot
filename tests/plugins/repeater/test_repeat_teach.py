...
def test_is_forced_repeat_teaching_requires_threshold_chain(monkeypatch) -> None:
    from src.plugins.repeater.repeat_teach import is_forced_repeat_teaching

    monkeypatch.setattr(
        "src.plugins.repeater.repeat_teach.repeat_ignore_user_ids",
        lambda: set(),
    )
...
def test_merge_forced_teach_weight_decays_and_accumulates() -> None:
    from src.features.persona.group_style_refresh import merge_forced_teach_weight
