...
def test_read_snowluma_runtime_webui_password(tmp_path: Path) -> None:
    ...
    (cfg_dir / "runtime.json").write_text(
        '{"webuiPort": 9000, "webuiPassword": "secret123"}', encoding="utf-8"
    )
...
def test_resolve_snowluma_prefers_logs_over_runtime_json(tmp_path: Path) -> None:
    ...
    cfg_dir.mkdir(parents=True)
    (cfg_dir / "runtime.json").write_text(
        '{"webuiPassword": "fromfile"}', encoding="utf-8"
    )
...
def test_resolve_snowluma_fallback_runtime_json(tmp_path: Path) -> None:
    cfg_dir = tmp_path / "config"
    cfg_dir.mkdir(parents=True)
    (cfg_dir / "runtime.json").write_text(
        '{"webuiPassword": "onlyfile"}', encoding="utf-8"
    )
    acc = {"account_data_dir": str(tmp_path)}
    assert resolve_snowluma_webui_temp_password(acc, []) == "onlyfile"
