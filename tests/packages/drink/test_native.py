...
def test_drink_native_handler_accepts_every_existing_exact_alias(text: str) -> None:
    assert DrinkNativeHandler().accepts(context(text)) is True


@pytest.mark.parametrize("text", ["牛牛喝", "牛牛醒醒", "牛牛干杯呀"])
def test_drink_native_handler_rejects_other_text(text: str) -> None:
    assert DrinkNativeHandler().accepts(context(text)) is False
...
def test_drink_native_handler_only_owns_drink_route() -> None:
    ...
    assert MessagePlanner(registry).plan(context("牛牛喝酒")).kind == "native"
    assert MessagePlanner(registry).plan(context("牛牛喝酒", route_modules=frozenset({"greeting"}))).kind == "legacy"
...
async def test_shadow_planning_has_no_permission_or_service_side_effects(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    drink = AsyncMock()
    monkeypatch.setattr("packages.drink.native.satisfies_command_permission", permission)
    monkeypatch.setattr("packages.drink.native.service.drink", drink)
    registry = NativeHandlerRegistry()
    registry.register(DrinkNativeHandler())
...
async def test_native_handler_checks_permission_then_defers_shared_service(
    ...
    monkeypatch.setattr("packages.drink.native.satisfies_command_permission", permission)
    monkeypatch.setattr("packages.drink.native.service.drink", drink)
    monkeypatch.setattr("packages.drink.native.service.sober_up", sober_up)
    bot = MagicMock()
    bot.send = AsyncMock()
    event = SimpleNamespace(self_id=1, group_id=2)

    outcome = await DrinkNativeHandler().handle(context(text), bot=bot, event=event)
    ...
    assert outcome.continue_legacy is (permission_id == "drink.drink")
    assert outcome.legacy_exclude_modules == (frozenset({"drink"}) if permission_id == "drink.drink" else frozenset())
    drink.assert_not_awaited()
...
async def test_denied_permission_falls_back_without_service_call(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("packages.drink.native.satisfies_command_permission", AsyncMock(return_value=False))
    drink = AsyncMock()
    monkeypatch.setattr("packages.drink.native.service.drink", drink)

    outcome = await DrinkNativeHandler().handle(context("牛牛喝酒"), bot=MagicMock(), event=MagicMock())
...
async def test_deferred_failure_does_not_turn_into_legacy_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("packages.drink.native.satisfies_command_permission", AsyncMock(return_value=True))
    monkeypatch.setattr(
        "packages.drink.native.service.drink",
