...
def test_startup_summary_lists_component_ready_scheduled_and_skipped_states() -> None:
    ...
    startup_report.reset_startup_report_for_tests()
    startup_report.register_startup_ready("入站调度", "调度并发 [8] | 发送 worker [4]")
    startup_report.register_startup_scheduled("语料预取", "worker [2]")
    ...
    assert "[入站调度] 已就绪：调度并发 [8] | 发送 worker [4]" in info_lines
    assert "[语料预取] 已调度：worker [2]" in info_lines
...
def test_plugin_ready_uses_custom_ops_detail() -> None:
    import pallas.core.foundation.startup_report as startup_report

    startup_report.reset_startup_report_for_tests()
    startup_report.register_plugin_startup_ready("maa", detail="MAA 远控 HTTP 路由已挂载")

    info_lines, warning_lines = startup_report.build_startup_summary_lines(base_lines=[])

    assert "[Ready] MAA 远控 HTTP 路由已挂载" in info_lines
    assert warning_lines == []
...
def test_format_helpers_cover_common_facts() -> None:
    ...
    assert startup_report._format_plugins(
        ...
        "已成功载入 41 个插件：本地 8 | 内置 12 | 官方 12 | NoneBot 3 | 社区 4 "
        "| 额外目录 2 | 配置跳过 2：内置 1 | NoneBot 1"
    )
