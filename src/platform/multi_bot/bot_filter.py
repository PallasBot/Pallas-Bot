...
from src.platform.multi_bot.connected_roster import connected_bot_ids, register_connected_roster_hooks
...
async def is_other_bot(event: GroupMessageEvent) -> bool:
    ...
    if await BotConfig(event.self_id, event.group_id).is_dreaming():
        return False
    from src.features.plugin_coord.duel import is_duel_paired_bot_traffic
