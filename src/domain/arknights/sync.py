...
from src.domain.arknights.duel_sync import build_operators_payload as build_operators_core
...
def build_operators_payload(
    ...
    *,
    handbook_lines_limit: int = 3,
) -> dict[str, Any]:
    payload = build_operators_core(char_table, skill_table)
    if not handbook_table:
        return payload
...
def run_arknights_sync(plan: ArknightsSyncPlan) -> ArknightsSyncResult:
    ...
    if plan.avatars_only:
        ...
        if need_handbook:
            result.messages.append("fetching handbook_table ...")
            handbook_table = fetch_json_sync(HANDBOOK_URL)
        if need_stage:
        ...
        if plan.operators:
            payload = build_operators_payload(
                char_table,
                skill_table,
                handbook_table if plan.handbook_enrich else None,
            )
