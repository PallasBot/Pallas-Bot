...
from src.domain.arknights.sync import load_enemies_payload


def list_operators(*, payload: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    data = payload if payload is not None else load_operators_payload()
    if not data:
        return []
...
def find_operator_raw(name: str, *, payload: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    for op in list_operators(payload=payload):
        if str(op.get("name", "")).strip() == needle:
            return op
    return None
