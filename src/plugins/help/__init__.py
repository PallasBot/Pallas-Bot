...
async def is_config_admin(event: GroupMessageEvent) -> bool:
    return await BotConfig(event.self_id).is_admin_of_bot(event.data.peer_id)
...
@help_cmd.handle()
async def handle_help_cmd(bot: Bot, event: GroupMessageEvent | FriendMessageEvent, state: T_State):
    """处理帮助命令（群聊和私聊）"""
    if isinstance(event, GroupMessageEvent):
        config = GroupConfig(event.data.peer_id, cooldown=3)
...
def extract_plugin_name_from_command(event: GroupMessageEvent | FriendMessageEvent, prefix: str) -> str:
    """从命令文本中提取插件名称"""
...
async def handle_enable_command(bot: Bot, event: GroupMessageEvent | FriendMessageEvent, state: T_State):
    """处理功能启用命令"""
...
async def handle_disable_command(bot: Bot, event: GroupMessageEvent | FriendMessageEvent, state: T_State):
    """处理功能禁用命令"""
...
async def toggle_all_plugins(bot: Bot, event: GroupMessageEvent | FriendMessageEvent, action: str, matcher):
    """处理启用/禁用所有功能的命令"""
...
async def handle_enable_all_command(bot: Bot, event: GroupMessageEvent | FriendMessageEvent):
    """处理启用所有功能的命令"""
    await toggle_all_plugins(bot, event, "enable", plugin_enable_all_cmd)


@plugin_disable_all_cmd.handle()
async def handle_disable_all_command(bot: Bot, event: GroupMessageEvent | FriendMessageEvent):
    """处理禁用所有功能的命令"""
