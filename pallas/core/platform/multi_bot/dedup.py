...
from pallas.core.platform.multi_bot.claim import read_claim_owner_sync, try_claim_message
    ...
    async with _shard_ingress_file_locks[lock_key]:
        ...
        if owner is not None:
            if owner == shard_id:
                return True
            if ingress_shard_claim_owner_obsolete(owner):
                from pallas.core.platform.multi_bot.claim import take_claim_message
...
async def try_begin_group_owned_gate(
    ...
    *,
    ...
    if sharding_active():
        from pallas.core.platform.shard.coord.group_gate import try_begin_owned_gate_sync
...
async def try_acquire_group_broadcast_slot(
    ...
    *,
    ...
    if sharding_active():
        from pallas.core.platform.shard.coord.group_gate import try_acquire_broadcast_slot_sync
...
def bind_group_owned_gate_sync(plugin: str, group_id: int, bot_id: int, *, gate_sec: float) -> None:
    ...
    if sharding_active():
        from pallas.core.platform.shard.coord.group_gate import bind_owned_gate_sync
...
def release_group_owned_gate_sync(plugin: str, group_id: int) -> None:
    ...
    if sharding_active():
        from pallas.core.platform.shard.coord.group_gate import release_owned_gate_sync
...
async def begin_group_exclusive_activity(
    ...
    *,
    ...
    from pallas.core.platform.shard.coord.group_activity import begin_group_activity, get_group_activity_lock
