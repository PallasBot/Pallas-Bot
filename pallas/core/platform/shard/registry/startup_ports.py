...
from pallas.core.platform.shard.registry.sync_protocol_ports import ProtocolPortSyncResult, sync_accounts_ws_urls
...
def evaluate_protocol_port_sync(
    *,
    ...
    if sync_result.onebot_sync_skipped_reason:
        notes.append(sync_result.onebot_sync_skipped_reason)
    if skip_protocol:
        if sync_result.onebot_sync_skipped_reason:
            notes.append("协议端 accounts.json ws_url 已对齐")
        else:
