    ...
    def plan(self, context: MessageContext) -> HandlingPlan:
        if not context.command_traffic:
            ...
            if len(handler_ids) == 1:
                return HandlingPlan(kind="direct", handler_ids=handler_ids, reason="unique_passive")
            reason = "chat_traffic" if not handler_ids else "multiple_passive_handlers"
            return HandlingPlan(kind="matcher", handler_ids=(), reason=reason)
        if not context.route_modules:
            return HandlingPlan(kind="matcher", handler_ids=(), reason="no_direct_route")
        ...
        if not handler_ids:
            ...
            return HandlingPlan(kind="matcher", handler_ids=(), reason=reason)
        if len(handler_ids) != 1:
            return HandlingPlan(kind="matcher", handler_ids=(), reason="multiple_direct_handlers")
        return HandlingPlan(kind="direct", handler_ids=handler_ids, reason="unique_command")
