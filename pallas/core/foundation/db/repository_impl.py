...
class MongoStickerLabelRepository:
    ...
    async def stats(self, *, min_confidence: float = 0.6, current_prompt_version: int | None = None) -> dict[str, int]:
        rows = await StickerLabel.get_pymongo_collection().find(
            {}, {"is_sticker": 1, "confidence": 1, "prompt_version": 1}
        ).to_list(None)
        total = len(rows)
    ...
    async def list_relabel_candidates(
        self, *, min_confidence: float, current_prompt_version: int, limit: int = 200
    ):
        from pallas.product.llm.sticker_labels import StickerSemanticLabel
