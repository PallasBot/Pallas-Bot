...
async def execute_corpus_prefetch(keywords: str) -> None:
    ...
    if should_pause_tasks():
        from pallas.core.platform.ingress.dispatch_metrics import record_prefetch_paused

        record_prefetch_paused()
        return
    from pallas.product.corpus.factory import build_community_repository
    from pallas.product.corpus.remote_budget import should_skip_remote_corpus
...
def prefetch_concurrency() -> int:
    from pallas.core.foundation.db.pool_budget import cap_by_pg_pool, remote_corpus_concurrency_limit
...
def bind_corpus_prefetch_lifecycle() -> None:
    ...
    async def _on_startup() -> None:
        if remote_corpus_find_mode() == "prefetch":
            await start_corpus_prefetch_workers()
            register_startup_ready("语料预取", f"worker [{len(_prefetch_tasks)}] | 队列上限 [{_QUEUE_MAX}]")
