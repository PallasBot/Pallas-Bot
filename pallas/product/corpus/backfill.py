...
from pallas.product.corpus.store import corpus_community_enrollment_valid
from pallas.product.corpus.text_util import plain_message_text
from pallas.product.corpus.write_fanout import corpus_write_queue
...
async def list_local_contexts_page(*, after_keywords: str, limit: int) -> list[Context]:
    backend = __import__("pallas.core.foundation.db", fromlist=["get_db_backend"]).get_db_backend()
...
async def list_local_contexts_page_pg(*, after_keywords: str, limit: int) -> list[Context]:
    ...
    from pallas.core.foundation.db.repository_pg import ContextRow, get_session, row_to_context
