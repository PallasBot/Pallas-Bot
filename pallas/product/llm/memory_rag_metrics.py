...
def stats_file_path():
    return plugin_data_dir("pb_webui", create=True) / "llm_memory_rag_stats.json"
...
def rollover_if_needed() -> None:
    ...
    if _day_key:
        try:
            ...
            write_day_side(_day_key, "ai", {"memory_rag": old, "day_key": _day_key, "source": "bot"})
        except Exception:
            pass
        _hit_count = 0
        _miss_count = 0
...
def record_memory_rag_query_result(
    *,
    hit: bool,
...
def merge_llm_memory_rag_snapshots(rows: list[dict[str, Any]]) -> dict[str, Any]:
    hit_count = 0
    miss_count = 0
...
def llm_memory_rag_metrics_snapshot(*, include_persisted: bool = True) -> dict[str, Any]:
    with _lock:
        rollover_if_needed()
...
def cluster_llm_memory_rag_metrics_snapshot(*, max_stale_sec: float = 300.0) -> dict[str, Any]:
    rows = [llm_memory_rag_metrics_snapshot(include_persisted=True)]
    try:
    ...
    if len(rows) <= 1:
        ...
        return out
    merged = merge_llm_memory_rag_snapshots(rows)
    merged["source"] = "bot_cluster"
    return merged


def flush_memory_rag_stats_sync() -> None:
    try:
        from pallas.core.platform.shard import context as shard_ctx

        if shard_ctx.sharding_active() and shard_ctx.is_worker():
            return
        snapshot = llm_memory_rag_metrics_snapshot(include_persisted=True)
    except Exception:
        snapshot = llm_memory_rag_metrics_snapshot(include_persisted=True)
    ...
    try:
        ...
        write_day_side(
            ...
            "ai",
            {"memory_rag": snapshot, "day_key": snapshot.get("day_key"), "source": "bot"},
        )
    except Exception:
        pass


def clear_llm_memory_rag_metrics_for_tests() -> None:
    global _day_key, _hydrated, _hit_count, _miss_count  # noqa: PLW0603
    with _lock:
        _day_key = today_key()
        _hydrated = True
