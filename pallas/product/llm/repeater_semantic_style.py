...
async def handle_repeater_semantic_style_backfill_scan(payload: dict[str, Any]) -> None:
    """在 work aux 扫描历史消息，并持久化生成的语义标注任务。

    本 handler 只负责入队回填 job，不产生要发送的 Bot 动作，故返回 None，
    避免 work aux 的 result_committer 把非 DirectWorkResult 返回值当成待提交动作。
    """
    if not SEMANTIC_STYLE_BACKFILL_ENABLED:
        return
