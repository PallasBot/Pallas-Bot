...
from pallas.core.platform.message_runtime.models import DeferredAction, HandlingOutcome, MessageContext, SendAction
...
def reply_outcome(context: MessageContext, message: object, operation: str, narrative: str) -> HandlingOutcome:
    async def log_sent_reply() -> None:
        logger.info(format_plugin_event(operation, narrative))

    return HandlingOutcome(
        handled=True,
        actions=(SendAction(message=message),),
        deferred_actions=(DeferredAction(name=f"pb_core.{operation}", run=log_sent_reply, wait_for_completion=True),),
    )
...
class StatusDirectHandler:
    ...
    async def handle(self, context: MessageContext, *, bot: Bot, event: Event) -> HandlingOutcome:
        ...
        return reply_outcome(
            context,
            format_runtime_status_text(self_id=context.bot_id),
            "show_status",
            f"Bot [{context.bot_id}] sent runtime status in group [{context.group_id}]",
        )
...
class ConsoleDirectHandler:
    ...
    async def handle(self, context: MessageContext, *, bot: Bot, event: Event) -> HandlingOutcome:
        ...
        return reply_outcome(
            context,
            format_console_hint_text(),
            "show_console",
            f"Bot [{context.bot_id}] sent the console hint in group [{context.group_id}]",
        )
...
class PluginsDirectHandler:
    ...
    async def handle(self, context: MessageContext, *, bot: Bot, event: Event) -> HandlingOutcome:
        ...
        return reply_outcome(
            context,
            format_plugins_summary_text(loaded_names=loaded),
            "list_plugins",
            f"Bot [{context.bot_id}] sent a list of [{len(loaded)}] loaded plugins in group [{context.group_id}]",
        )
