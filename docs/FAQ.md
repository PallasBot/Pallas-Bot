...
### Q: 启动报 `Command buildInfo requires authentication` / `Unauthorized`？

A: MongoDB **已开启认证**，但 `.env` 里 **`MONGO_USER` / `MONGO_PASSWORD` 为空或不正确**。处理方式任选其一：

- 在 `.env` 填写 **`MONGO_USER`**、**`MONGO_PASSWORD`**；若用户建在 `admin` 库，增加 **`MONGO_AUTH_SOURCE=admin`**（未设置时默认与 **`MONGO_DB`** 相同）。
- 若本机 Mongo 无需认证：检查是否用了带鉴权的旧数据目录，或误连到远程实例。
- 已迁移到 PG 时：设置 **`DB_BACKEND=postgresql`** 并配置 **`PG_*`**，勿再使用 `mongodb` 后端。
