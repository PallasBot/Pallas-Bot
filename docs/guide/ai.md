...
3. **自备唱歌音色**：放到 AI 仓 `resource/sing/models/<音色名>/`（目录名即 Speaker id，不要用 `pretrain`），刷新「唱歌」页应出现在 Speaker 列表；DDSP / RVC / SoVITS 文件约定见 [唱歌插件说明](../plugins/sing/README.md#自备音色)
