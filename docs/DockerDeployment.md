...
- 若你**自行**把 NapCat 写成与 `pallasbot` **同一 Compose 网络**的 service，则可在 NapCat 里把 OneBot 客户端 URL 写成 **`ws`** + **`://`** + **`pallasbot:<PORT>/onebot/v11/ws`**（常见内网 OneBot 走非 TLS）；这与当前协议端插件 **`docker run` 默认网络** 是两条路径。
