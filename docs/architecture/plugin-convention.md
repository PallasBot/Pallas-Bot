...
- **插件存储键**：在 `extra["plugin_storage"]` 声明群/用户/牛级数据键；读写使用 `src.features.plugin_storage.GroupPluginStorage` 或 `get/set_plugin_storage`（见 [develop/plugin/structure.md](../develop/plugin/structure.md)）。
