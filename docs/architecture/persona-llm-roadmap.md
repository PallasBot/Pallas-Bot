...
> **目标版本：4.0**（见 [pallas-4.0-roadmap](pallas-4.0-roadmap.md)）。语言层 LLM **运行时依赖 [Pallas-Bot-AI](pallas-ai-service.md)**；主仓 `features/llm` 负责 persona、工具与业务路由，AI 仓 4.0 并行重构统一网关。
...
compile_group_style_snapshot/profile  # 群风格契约（已实现）
      ↓
compile_persona_prompt(bot, group)  # 待实现：system / memory 总装
      ↓
features/llm chat_complete            # 待实现：OpenAI 兼容调用
      ↓
repeater fallback / polish            # 待实现：语料 miss / 轻改写，默认关
```
...
| 能力 | 说明 | 阶段 |
| --- | --- | --- |
| `compile_persona_prompt` | 合并牛格、`compile_group_style_prompt`、帕拉斯基础 system；可选 `role` / `knowledges` / `bot_self` 结构 | P1 |
| LLM 运行时 | 主仓 `features/llm` → **AI 仓**统一 Chat API；**local + remote provider chain** 在 AI 仓配置（见 [pallas-ai-service · Provider](pallas-ai-service.md#provider本地--远端--备线)） | P2 |
| 会话记忆 | 群 / 用户双层存储、滑动窗口、TTL；分片共享 PG | P3 |
| 治理 | 每 bot/群开关、CD、异步队列、token 预算、`SlowPathTimer` 观测 | P4 |
| 回复决策 | 可选：ingress 或 miss 分支由 LLM 判断 `should_reply`（默认关） | 后期 |
| 长期记忆 | embedding 检索（群 / 用户事实），与结构化 KB 分离 | P8 |
| 工具调用 | 预分类后按需注入 Tool Schema；插件 `extra["llm_tools"]` 注册 | P9 |
| 多模态 / 联网 | 视觉模型、MoE 路由、外部搜索（均默认关） | P10 |
...
| 待实现能力 | 可对齐的本仓模块 | 说明 |
| --- | --- | --- |
| P2 provider / HTTP | [pallas-ai-service](pallas-ai-service.md)、`ollama/chat_message.py` | 现经 AI 仓 `:9099`；4.0 改统一 LLM 路径 |
| P3 会话存储 | `src/foundation/db/`、`ollama` 会话 id | PG repository 模式；ollama 现用 `event.get_session_id()` |
| P4 CD / 限流 | `src/features/command_limits/` | 分片 hub 聚合冷却；LLM 可复用同一 helper |
| P4 慢路径观测 | `src/platform/observability/SlowPathTimer` | repeater / ingress 已在用 |
| P4 配置热重载 | WebUI + `install_hot_reload_config` | 见 [webui 配置热重载](../common/webui/README.md) |
| P5–P6 repeater 挂钩 | `src/plugins/repeater/responder.py`、`learn_queue.py` | miss 分支与异步队列；不挡 learn |
...
| P2 | `src/features/llm/`：调用 **AI 仓统一 LLM API**（见 [pallas-ai-service](pallas-ai-service.md)）；主仓不直连 Ollama | 关 |
| P3 | PG 会话：群窗口 N 条 + 可选用户 TTL；分片 worker 共享读 | — |
| P4 | 每 bot/群开关、CD、异步队列、token 预算 | 关 |
| P5 | `_context_find` miss 且开关开 → LLM 生成 | **关** |
| P6 | 已有候选句 → LLM 轻改写；失败回退原句 | **关** |
...
2. **默认关**：`llm_fallback_enabled`、`llm_polish_enabled` 与 repeater `fanout_enabled` 同理，WebUI 可配。
...
| `src/features/llm/`（新建） | Chat 客户端 → **Pallas-Bot-AI**；会话 CRUD |
| `Pallas-Bot-AI`（**并行仓库**） | 统一 LLM 网关、会话、provider；见 [pallas-ai-service](pallas-ai-service.md) |
| `src/plugins/repeater/responder.py` | fallback / polish 钩子（默认关） |
...
| `src/plugins/ollama/` | **legacy** `@牛牛` 入口；4.0 内转 `features/llm`，长期迁 `llm_*` 配置 |
...
- [ ] staging 固定 prompt 调通 Chat API（不接群消息）
- [ ] `compile_persona_prompt` 输出可人工 review
- [ ] 分片多 worker 会话读写无重复回复
- [ ] 高并发下 CD/队列不打爆 PG 池与 API 配额

### LLM 行为（P5–P6）

- [ ] fallback 开：语料 miss 可生成；关：与现网一致
- [ ] polish 开：句长/语气贴近群 `length_pref`；失败回退原句
...
- [pallas-ai-service.md](pallas-ai-service.md) — Bot ↔ AI 仓 4.0 协同
