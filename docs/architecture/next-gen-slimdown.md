...
## Phase 2 — platform 收敛（当前可开工）

**目标**：减少 `src/platform/shard/` 与插件内分散的 `is_sharding_active` 分支。

| 任务 | 做法 | 验收 |
| --- | --- | --- |
| shard context API | 新增 `src/platform/shard/context.py`：`role()`、`sharding_active()`、`representative_bot()` | 新代码不直接 import 20+ shard 模块 |
| coord listener 注册表 | `worker_poll` 改为插件式 `{starter}` 列表 | 新 coord 不改 central 文件 |
| 合并 ai_task 双层 | `ai_task_registry_redis` 并入 `ai_task_registry` | 无「文件回退」注释 |
| deploy extra 澄清 | `pyproject.toml`：`shard=[]` 标注弃用，文档指向 `deploy-shard` | 误装无 Redis 环境减少 |
| `prune_shard_coord.py` | 重命名为 `shard_coord_snapshot.py` 或合并进 observability | 运维脚本名与行为一致 |

**非目标（本阶段不做）**：删除 `ingress_gate`、砍掉分片测试、移除 `registry.json`。
...
| 优先级 | 范围 | 方向 |
| --- | --- | --- |
| P0 | `help`、`repeater`、`bot_status` | 已部分元数据化；继续删重复 preprocessor / fanout 判定 |
