...
def main() -> None:
    ...
    priority_tiers = (
        ...
        frozenset({"repeater.message"}),
        frozenset({"repeater.learn"}),
    )
    asyncio.run(
        run_work_service(load_work_handlers(), exclude_kinds=frozenset({SEND_JOB_KIND}), priority_tiers=priority_tiers)
    )
