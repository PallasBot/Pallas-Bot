...
- 群消息处理升级为统一消息运行时：命令、被动回复与跨部署转发走同一套原生入口；多牛牛接话、延迟回复和发送失败归因更稳定，排查消息未发出也更直接。
- 表情回应升级为可维护的语义标签：缓存表情会识别场景、沉淀标签并按语义召回，必要时再精排；控制台可查看和调整群表达与表情标签。
- 闲聊人格改为“背景事实 + 真实群聊习惯”分层：保留方舟资料回答，但日常不再搬角色设定；可爱和互动来自自然短句，不靠世界观台词或固定卖萌。
- AI 接话支持结构化短气泡：会先按当前情绪或话题给出即时反应，再按需要分条补充，减少整段独白、戏剧化转折和客服式收尾。
- 回复可引用近期群消息：命中明确上下文时自动带 QQ 引用，群内多人同时聊天时更容易接对正确的内容。
- 新增数据库生命周期管理：控制台可查看对象规模和保留策略，预览清理影响后确认执行；默认每日维护，PG 查询统计默认关闭以降低开销。
- Docker 部署可从控制台检查并覆盖更新 Bot 与协议镜像；非 Docker 环境提供带内置 WebUI 的精简部署包。
- 更新页改从 WebUI 仓库获取控制台版本，与 Bot 版本解耦；自动更新私聊会列出 Bot、WebUI 和插件版本区间及重启安排。
- `#pallas` 状态命令开放给群管、群主和牛牛号主；状态回复不再展示本进程连接账号列表。
- 控制台补齐慢接口缓存、插件来源与版本展示、代理歌曲缓存策略和窄屏布局优化。
- 捆绑控制台 WebUI v0.9.1。
...
- 提供方可注册多个模型，并按模型配置请求价格条件；控制台选择默认模型后可直接用于任务编排。
- 价格条件支持输入 Token 闭区间、每日时段、按 Token 与按次计费；命中条件后按整次请求结算并留存账单快照。
...
- 闲聊补充分层表达指导与人格锚点，短对话、称呼和回应方式更贴近帕拉斯。
- 接话语义风格支持学习与运行配置，复读、表情回应和缓存表情图的互动更自然。
- 入站调度按实际执行容量与会话负载分配资源；控制台可查看近期趋势、学习流水线与容量状态。
- 修复后台任务并发竞态、命令被工具接管、唤醒确认过滤及多机协调报数等稳定性问题。
- 捆绑控制台 WebUI v0.8.29。
...
- 群内明确 @ 某只本机牛牛时，跨部署协调不再让远端抢占，问候和对话会稳定由目标账号响应。
- 捆绑控制台 WebUI v0.8.28。
...
- 高峰时按会话公平调度，并优先保障命令处理，减少热点群占用消息处理资源。
- 复读学习和持久化任务改为受控后台执行，支持并发处理、续租与异常任务隔离，消息回复更稳定。
- 高压下保留分级推理能力，并提供 LLM 预算、入站调度和后台任务运行指标。
- 统一运行时支持后台重启，辅进程启动失败时可安全回滚。
...
- 默认统一运行时可接入本机全部账号，以会话公平调度、命令优先和背压处理高峰，不再因账号数量要求分片。
- 新增 work aux 执行学习和持久化等后台任务，消息回复不再等待这些重活完成。
- 单机可继续不使用 Redis；多机协调、本机 Embedding 和 AI 队列可按需复用同一 Redis。
- 报数改为后台依次发送，不再阻塞同群后续消息；多机报数请同步升级 Bot 与 pallas-plugin-bot-status >= 4.0.22。
- 控制台可查看入站调度与辅进程日志，定位高峰积压更直接。
- 捆绑控制台 WebUI v0.8.26。
...
- 每个牛牛账号可保存独立的处事风格，回复中的主动程度、分歧处理和偏好更符合账号设定
- 长期记忆增加治理元数据与范围过滤，减少不相关记忆进入当前对话
- 工具调用会结合语义线索、执行约束和命令素材来源；耗时任务可恢复查看进度
