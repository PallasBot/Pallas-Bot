...
- 闲聊与表情回应更自然：语义风格按节奏学习、相似接话优先注入，纯文本表情标记可点名表情包，短气泡兜底按换行拆段。
- 表情标签更稳健：宽松解析、每日预算回填、审核拒绝记负标签，永久错误不再熔断或无限重试。
- 修复数据库生命周期维护：未配置分片协调 Redis 时不再误判为“其他分片在执行”，PG 后端容量上限清理真正生效。
- 收尾更稳：数据库最后关闭、断连静默，Ctrl+C 不再刷屏。
- 日志与观测更清晰：日志模块名与业务前缀按来源稳定配色并限频输出，runtime 分阶段耗时与入站分路径指标可观测。
