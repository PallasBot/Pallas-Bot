...
- **智能对话**：回复更有人味——人设改为正面引导（少写“不要”，多说“你可以”），接梗、回顶、亲昵落点自然推进，不再套固定三步流程；群聊时间线去掉消息编号、改成带说话人的连续对话，机器人更容易“认出是谁在说话”。
- **智能对话**：提高基础采样温度并支持“别拿…说事”等固定回顶句式的规避，同一场景不再反复冒出同一句话。
- **人设**：短社交轮（调侃、夸夸、吐槽）现在也会读取与对话者的关系备注，机器人对被熟人时更放得开。
- **知识库**：新增按 bot 隔离的知识源（scope=bot），不同账号可以拥有彼此独立的私有知识。
...
- **记忆**：收紧自动事件记忆——默认改为从完整群消息提炼「群共同事件」摘要，不再逐条记录群消息原话；加入寒暄/碎片/临时信号的过滤，并新增每日提炼预算（`LLM_MEMORY_AUTO_EPISODE_DAILY_BUDGET`，默认 30），历史噪音记忆已清理。
