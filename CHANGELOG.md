...
* fix(llm): 记忆与摘要任务失败时同样计入每日预算
* fix(llm): 记忆与后台任务并入低档编排
* fix(llm): 语义风格视觉标注对齐编排任务键
...
- **智能对话**：
  - LLM 与 Embedding 后台出口统一遵守总开关及插件禁用状态
  - 共享 HTTP 客户端连续连接池超时后自动重建，避免只能重启 Bot 恢复
  - 每日预算按北京时间自然日计算
  - 视觉任务适配思考模型的输出预算
  - 行为模式统计与实际召回条件对齐
- **后台任务**：
  - 大积压下按优先级分桶领取，交互任务不再被复读任务挤压
- **权限与稳定性**：
  - 删除 WebUI 配置时同步撤销派生封禁权限，并清理历史孤儿规则
  - 修复拆包后的慢数据库会话调用方归因
- **复读**：
  - 「不可以发这个」无引用消息时仍可正常处理
- 捆绑控制台 WebUI v0.9.24
...
* fix(work_jobs): 大积压下领取任务改按优先级分桶，避免全量排序
* fix(llm): 连续池超时后重建共享 httpx 客户端
* fix(llm): 行为模式可注入统计对齐召回条件
* fix(llm): 每日预算改用北京自然日
* fix(llm): 视觉任务输出预算适配思考模型
* fix(llm): 内部任务输出预算统一走任务键
* fix(acl): 删除配置行时撤销 ACL 并清理历史孤儿规则
* fix(db): 慢会话调用方归因匹配拆包后的路径
* fix(repeater): 「不可以发这个」免引用不再报错

### Added

* feat(llm): LLM 出口统一受插件禁用与总闸约束
...
- **帮助菜单**：
  - 默认改用 HTML 渲染，观感更清晰统一
  - 二级菜单展示命令权限等级，停用插件状态更直观
  - 内置插件补充功能分组，布局与配色进一步优化
- **智能对话**：
  - 支持 Ollama 原生接口，思考内容不再混入回复正文
  - 工具查询设次数上限，重复调用自动拦截，查询较慢时先回「我查一下」
  - 长回复改多气泡分条投递，超长内容不再被压短或静默
  - 模型空输出不再硬凑垫词，保持静默
  - 仅群聊触发智能对话
- **语义风格与群洞察**：
  - 语义标注与记忆抽取加后台并发限制，高峰不挤占对话
  - 协议采集支持不引用直接回复的配对，并按群隔离、校验提示发送者
  - 修复对照组实验数据误计入实验桶的问题
- **记忆与预算**：
  - 记忆抽取按调用方拆分独立每日预算，互不挤占
- **其他**：
  - 插件详情 README 支持回退读取本地插件源码
  - 补齐私聊消息规范日志，启动不再重复播报连接数量
  - 移除过宽入站拦截词，减少误伤
- 捆绑控制台 WebUI v0.9.23（语义风格状态展示与 v2 回滚维护）

### Added

* feat(llm): 支持 Ollama 原生请求方式
* feat(llm): 开放社区只读工具注册（register_llm_tool）
* feat(llm): 语义标注与记忆抽取后台并发闸
* feat(llm): 记忆抽取按调用方拆分并补独立每日预算
* feat(llm): 定价规则支持多时段区间
* feat(llm): 完善工具查询与结果投递闭环
* feat(insight): 语义风格受控模式与实验治理
* feat(insight): 协议采集支持非引用回复配对
* feat(help): 默认使用 HTML 渲染帮助菜单
* feat(help): 为内置插件补充功能分组
* feat(webui): 插件详情页 README 回退读取 local/plugins
* feat(logging): 补齐私聊消息正规日志
* chore(templates): 扩展模板补全插件骨架

### Changed

* refactor(llm): 提取记忆写入的每日预算与冷却计时公共类
* docs(workflow): 统一文档写作与链接规范

### Fixed

* fix(help): 二级帮助菜单展示权限等级
* fix(help): 恢复帮助图浅色配色
* fix(help): 调整停用插件状态样式
* fix(help): 优化帮助菜单布局
* fix(llm): 限制智能对话仅群聊触发
* fix(llm): ollama 思考内容不再混入回复正文
* fix(llm): 配置方默认模型自动进注册列表
* fix(llm): 撤销直接触发空回复兜底
* fix(llm): 统一长回复多气泡投递
* fix(llm): 召回通用知识与 PRTS 剧情查询工具
* fix(llm): 移除博士通用过滤
* fix(llm): 放宽工具回答长度上限
* fix(llm): 确保工具轮次最终收尾
* fix(insight): 对照组 exposure 计入 control 桶
* fix(insight): 协议模式按群隔离并校验提示发送者
* fix(insight): 提高语义标注 token 预算
